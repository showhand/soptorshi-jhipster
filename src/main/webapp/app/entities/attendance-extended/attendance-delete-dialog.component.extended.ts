...
@Component({
    ...
    templateUrl: './attendance-delete-dialog.component.extended.html'
})
export class AttendanceDeleteDialogComponentExtended extends AttendanceDeleteDialogComponent {
...
export class AttendanceDeletePopupComponentExtended extends AttendanceDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ attendance }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(AttendanceDeleteDialogComponentExtended as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
