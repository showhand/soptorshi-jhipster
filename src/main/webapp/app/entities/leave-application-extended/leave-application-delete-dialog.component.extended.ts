...
@Component({
    ...
    templateUrl: './leave-application-delete-dialog.component.extended.html'
})
export class LeaveApplicationDeleteDialogComponentExtended extends LeaveApplicationDeleteDialogComponent {
...
export class LeaveApplicationDeletePopupComponentExtended extends LeaveApplicationDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ leaveApplication }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(LeaveApplicationDeleteDialogComponentExtended as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
