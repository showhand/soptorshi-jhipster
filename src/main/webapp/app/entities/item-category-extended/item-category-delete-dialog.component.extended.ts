...
@Component({
    ...
    templateUrl: './item-category-delete-dialog.component.extended.html'
})
export class ItemCategoryDeleteDialogComponentExtended extends ItemCategoryDeleteDialogComponent {
    itemCategory: IItemCategory;

    constructor(
        protected itemCategoryService: ItemCategoryServiceExtended,
...
export class ItemCategoryDeletePopupComponentExtended extends ItemCategoryDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ itemCategory }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(ItemCategoryDeleteDialogComponentExtended as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
