...
export class PeriodCloseComponent implements OnInit, OnDestroy {
    ...
    trackId(index: number, item: IPeriodClose) {
        return item.id;
    }

    registerChangeInPeriodCloses() {
        this.eventSubscriber = this.eventManager.subscribe('periodCloseListModification', response => this.reset());
    }
    ...
    protected paginatePeriodCloses(data: IPeriodClose[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.periodCloses.push(data[i]);
        }
    }
