...
export class WeekendComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.weekendService
                ...
                .subscribe(
                    (res: HttpResponse<IWeekend[]>) => this.paginateWeekends(res.body, res.headers),
    ...
    trackId(index: number, item: IWeekend) {
        return item.id;
    }

    registerChangeInWeekends() {
        this.eventSubscriber = this.eventManager.subscribe('weekendListModification', response => this.reset());
    }
    ...
    protected paginateWeekends(data: IWeekend[], headers: HttpHeaders) {
