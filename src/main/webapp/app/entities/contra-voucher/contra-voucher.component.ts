...
export class ContraVoucherComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.contraVoucherService
                ...
                .subscribe(
                    (res: HttpResponse<IContraVoucher[]>) => this.paginateContraVouchers(res.body, res.headers),
    ...
    ngOnInit() {
        ...
        this.registerChangeInContraVouchers();
    }
    ...
    trackId(index: number, item: IContraVoucher) {
        return item.id;
    }

    registerChangeInContraVouchers() {
        this.eventSubscriber = this.eventManager.subscribe('contraVoucherListModification', response => this.loadAll());
    }
    ...
    protected paginateContraVouchers(data: IContraVoucher[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.contraVouchers = data;
    }
